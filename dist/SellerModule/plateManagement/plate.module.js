"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.PlateModule = void 0;
const common_1 = require("@nestjs/common");
const mongoose_1 = require("@nestjs/mongoose");
const plate_schema_1 = require("../models/plate.schema");
const purchase_schema_1 = require("../../BuyerModule/models/purchase.schema");
const session_schema_1 = require("../../BuyerModule/models/session.schema");
const orderDetails_schema_1 = require("../models/orderDetails.schema");
const user_schema_1 = require("../models/user.schema");
const plateManagement_controller_1 = require("./plateManagement.controller");
const plateMangement_service_1 = require("./plateMangement.service");
const sendgrid_service_1 = require("../../sendgrid/sendgrid.service");
const config_1 = require("@nestjs/config");
const scheduler_service_1 = require("../../scheduler/scheduler.service");
let PlateModule = class PlateModule {
};
PlateModule = __decorate([
    (0, common_1.Module)({
        imports: [
            mongoose_1.MongooseModule.forFeature([{ name: 'Plate', schema: plate_schema_1.PlateSchema }]),
            mongoose_1.MongooseModule.forFeature([{ name: 'Order', schema: orderDetails_schema_1.OrderSchema }]),
            mongoose_1.MongooseModule.forFeature([{ name: 'Purchase', schema: purchase_schema_1.PurchaseSchema }]),
            mongoose_1.MongooseModule.forFeature([{ name: 'User', schema: user_schema_1.UserSchema }]),
            mongoose_1.MongooseModule.forFeature([{ name: 'Session', schema: session_schema_1.SessionSchema }]),
        ],
        providers: [
            plateMangement_service_1.PlateManagementService,
            sendgrid_service_1.SendGridService,
            config_1.ConfigService,
            scheduler_service_1.SchedulerService,
        ],
        controllers: [plateManagement_controller_1.PlateManagementController],
        exports: [],
    })
], PlateModule);
exports.PlateModule = PlateModule;
//# sourceMappingURL=plate.module.js.map